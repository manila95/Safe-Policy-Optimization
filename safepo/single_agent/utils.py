...
def rollout_policy(
    ...
    episode_data = {
        ...
        'value_r_v2': [],
        'value_c_v2': [],
        'cdq_r': [],  # min of the two reward critics
        'cdq_c': []   # min of the two cost critics
    }
    ...
    while episodes_completed < num_episodes:
        ...
        for _ in range(max_ep_len):
            with torch.no_grad():
                ...
                if use_risk:
                    value_r = policy.reward_critic(obs, risk)
                    value_r_v2 = policy.reward_critic_v2(obs, risk)
                    value_c = policy.cost_critic(obs, risk)
                    value_c_v2 = policy.cost_critic_v2(obs, risk)
                else:
                    value_r = policy.reward_critic(obs)
                    value_r_v2 = policy.reward_critic_v2(obs)
                    value_c = policy.cost_critic(obs)
                    value_c_v2 = policy.cost_critic_v2(obs)
                
                # CDQ is the min of the two critics
                cdq_r = torch.min(value_r, value_r_v2)  # min of reward critics
                cdq_c = torch.min(value_c, value_c_v2)  # min of cost critics
                
                episode_value_r.append(value_r)
                episode_value_r_v2.append(value_r_v2)
                episode_value_c.append(value_c)
                episode_value_c_v2.append(value_c_v2)
                episode_cdq_r.append(cdq_r)
                episode_cdq_c.append(cdq_c)
            ...
            for env_idx in done_envs:
                if episodes_completed < num_episodes:
                    ...
                    episode_data['value_r_v2'].append(torch.stack([vr[env_idx] for vr in episode_value_r_v2]))
                    episode_data['value_c_v2'].append(torch.stack([vc[env_idx] for vc in episode_value_c_v2]))
                    episode_data['cdq_r'].append(torch.stack([cdq_r[env_idx] for cdq_r in episode_cdq_r]))
                    episode_data['cdq_c'].append(torch.stack([cdq_c[env_idx] for cdq_c in episode_cdq_c]))
...
def create_critic_comparison_plot(
    critic_v1_values: torch.Tensor,
    critic_v2_values: torch.Tensor,
    title: str,
    value_type: str = "Reward"
) -> plt.Figure:
    """
    Create a scatter plot comparing predictions from critic v1 vs critic v2.
    
    Args:
        critic_v1_values: Tensor of values from critic v1
        critic_v2_values: Tensor of values from critic v2
        title: Plot title
    ...
    """
    # Convert to numpy
    v1_np = critic_v1_values.detach().cpu().numpy().flatten()
    v2_np = critic_v2_values.detach().cpu().numpy().flatten()
    
    # Calculate discrepancy
    discrepancy = v1_np - v2_np
    ...
    scatter = plt.scatter(v1_np, v2_np, c=discrepancy, alpha=0.4, cmap='seismic', s=10)
    plt.colorbar(scatter, label='Discrepancy (V1 - V2)')
    
    # Add diagonal line (y=x) for perfect agreement
    min_val = min(v1_np.min(), v2_np.min())
    max_val = max(v1_np.max(), v2_np.max())
    plt.plot([min_val, max_val], [min_val, max_val], 'k--', linewidth=2, label='Perfect Agreement (y=x)')
    
    # Calculate correlation
    corr = calculate_correlation(critic_v1_values, critic_v2_values)
    
    # Add title and labels
    plt.title(f'{title}\nMean Discrepancy: {mean_discrepancy:.4f} ± {std_discrepancy:.4f}\n'
              f'Pearson: {corr["pearson_corr"]:.3f}, Spearman: {corr["spearman_corr"]:.3f}')
    plt.xlabel(f'Critic V1 {value_type} Value', fontsize=11)
    plt.ylabel(f'Critic V2 {value_type} Value', fontsize=11)
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    # Add text annotation about interpretation
    if mean_discrepancy > 0:
        bias_text = f'Critic V1 overestimates by {mean_discrepancy:.4f} on average'
    else:
        bias_text = f'Critic V1 underestimates by {abs(mean_discrepancy):.4f} on average'
...
def evaluate_critic_performance_from_rollouts(
    ...
    all_value_r_v2 = torch.cat(episode_data['value_r_v2'])
    all_value_c_v2 = torch.cat(episode_data['value_c_v2'])
    all_cdq_r = torch.cat(episode_data['cdq_r'])
    all_cdq_c = torch.cat(episode_data['cdq_c'])
    all_reward_returns = torch.cat(returns['reward_returns'])
    all_cost_returns = torch.cat(returns['cost_returns'])

    # Optional discrepancy for color-coding
    reward_color_values = torch.abs(all_value_r - all_value_r_v2)
    cost_color_values = torch.abs(all_value_c - all_value_c_v2)
    ...
    reward_metrics = evaluate_value_estimation_error(
        ...
        color_label="Reward Discrepancy |V1 - V2|"
    )
    ...
    cost_metrics = evaluate_value_estimation_error(
        ...
        color_label="Cost Discrepancy |V1 - V2|"
    )
    ...
    # Evaluate reward CDQ (min of two reward critics) against MC returns
    cdq_r_metrics = evaluate_value_estimation_error(
        all_cdq_r,
        all_reward_returns,
        create_plot=create_plots,
        plot_title="Reward CDQ (min of reward critics) vs MC Returns",
        color_values=None,
        color_label=""
    )
    
    # Evaluate cost CDQ (min of two cost critics) against MC returns
    cdq_c_metrics = evaluate_value_estimation_error(
        all_cdq_c,
        all_cost_returns,
        create_plot=create_plots,
        plot_title="Cost CDQ (min of cost critics) vs MC Returns",
        color_values=None,
        color_label=""
    )
    ...
    result = {
        ...
        'cdq_r_critic': cdq_r_metrics,
        'cdq_c_critic': cdq_c_metrics
    }
    
    # Compute discrepancies between the two critics
    # Discrepancy = critic_v1 - critic_v2
    # Positive means critic_v1 overestimates relative to critic_v2
    reward_discrepancy = all_value_r - all_value_r_v2
    cost_discrepancy = all_value_c - all_value_c_v2
    ...
    reward_discrepancy_metrics = {
        ...
        'mean_critic_v1_value': all_value_r.mean().item(),
        'mean_critic_v2_value': all_value_r_v2.mean().item(),
    }
    ...
    cost_discrepancy_metrics = {
        ...
        'mean_critic_v1_value': all_value_c.mean().item(),
        'mean_critic_v2_value': all_value_c_v2.mean().item(),
        'mean_cdq_c_value': all_cdq_c.mean().item(),
    }
    ...
    if create_plots:
        reward_comparison_fig = create_critic_comparison_plot(
            all_value_r,
            all_value_r_v2,
            title=f"Reward Critic Comparison: V1 vs V2",
            value_type="Reward"
        )
        ...
        cost_comparison_fig = create_critic_comparison_plot(
            all_value_c,
            all_value_c_v2,
            title=f"Cost Critic Comparison: V1 vs V2",
            value_type="Cost"
        )
