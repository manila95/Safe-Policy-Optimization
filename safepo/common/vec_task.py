...
class VecTask:
    def __init__(self, task, rl_device, clip_observations=5.0, clip_actions=1.0):
        ...
        self.obs_space = task.observation_space #spaces.Box(np.ones(self.num_obs) * -np.Inf, np.ones(self.num_obs) * np.Inf)
        self.state_space = spaces.Box(
            np.ones(self.num_states) * -np.Inf, np.ones(self.num_states) * np.Inf
        )
        self.act_space = task.act_space#spaces.Box(
...
class VecTaskPython(VecTask):
    ...
    def step(self, actions):
        ...
        obs_buf, rew_buf, reset_buf, _ = self.task.step(actions_tensor)
    ...
    def reset(self):
        ...
        # step the simulator
        obs_buf, rew_buf, reset_buf, _ = self.task.step(actions)
