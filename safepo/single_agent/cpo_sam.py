...
def main(args, cfg_env=None):
    ...
    cost_critic_optimizer = SAM(
        policy.cost_critic.parameters(), torch.optim.Adam, rho=0.05, lr=1e-3
    )
    ...
    for epoch in range(epochs):
        ...
        for _ in range(config["learning_iters"]):
            for (
                ...
                # Use SAM for reward critic
                def reward_critic_closure():
                    value_pred = policy.reward_critic(obs_b, risk_b)
                    loss = nn.functional.mse_loss(value_pred, target_value_r_b)
                    if config.get("use_critic_norm", True):
                        for param in policy.reward_critic.parameters():
                            loss += param.pow(2).sum() * 0.001
                    return loss
                
                reward_critic_optimizer.zero_grad()
                reward_critic_optimizer.step(reward_critic_closure)
                
                # Use SAM for cost critic
                def cost_critic_closure():
                    value_pred = policy.cost_critic(obs_b, risk_b)
                    loss = nn.functional.mse_loss(value_pred, target_value_c_b)
                    if config.get("use_critic_norm", True):
                ...
                cost_critic_optimizer.step(cost_critic_closure)

                logger.store(
                    **{
                        "Loss/Loss_reward_critic": reward_critic_closure().item(),
                        "Loss/Loss_cost_critic": cost_critic_closure().item(),
                    }
                )
